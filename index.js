...
function filterStudentsByGrade(students, grade) {
  ...
  for (const obj of students) {
    if (obj.grade === grade) {
      console.log(`Студент ${obj.name} має оцінку ${obj.grade}`);
    };
  };
}
...
function validateUserInput(userInput) {
  if (userInput.username.length > 0) {
    ...
    if (userInput.password.length > 0) {
      // Перевірка довжини паролю чи менше вона ніж 8,якщо ні виводимо повідомлення про потенційну проблему з текстом
